...
class Text(
    ...
    fun getClientName(i: Int,i2: Int): String{
        return (LiquidBounce.CLIENT_NAME).substring(i,i2);
    }
    /**
     * Draw element
     */
    override fun drawElement(partialTicks: Float): Border {
        ...
        if(!rectValue.get().contains("Logo")) {
            ...
            FontLoaders.F40.drawString(
                ...
                getClientName(0,6), 5F, 0F,Color(255,255,255,180).rgb
            )
            FontLoaders.C16.drawString(
                getClientName(6,12), 5F + FontLoaders.F40.getStringWidth(LiquidBounce.CLIENT_NAME_SHORT), 13F,Color(255,255,255,180).rgb
            )
            RenderUtils.drawRect(5f,22.5f,90f,22.8f,Color(200,200,200,120).rgb)
            FontLoaders.C14.drawString(
                LiquidBounce.CLIENT_VERSION + " | "+LiquidBounce.VERSIONTYPE, 5F, 27F,Color(255,255,255,180).rgb
            )
            FontLoaders.C14.drawString(
                LiquidBounce.BUILDTIPS, 5F, 37F,Color(255,255,255,180).rgb
            )
        }
