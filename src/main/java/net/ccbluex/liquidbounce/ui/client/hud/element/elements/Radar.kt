...
import net.ccbluex.liquidbounce.utils.ColorsAR
...
class Radar : Element() {
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        if (exhiValue.get())
            arRenderUtils.drawExhiRect(0F, if (lineValue.get()) -1F else 0F, size, size)
        else
            arRenderUtils.drawRoundedRect(0F, 0F, size, size,bordRad.get(), Color(backgroundRedValue.get(), backgroundGreenValue.get(),
        ...
        if (lineValue.get()) {
            ...
            for (i in 0..(gradientAmountValue.get()-1)) {
                ...
                arRenderUtils.drawGradientSideways(barStart, -1.0, barEnd, 0.0,
                    when (rainbowType) {
                        "CRainbow" -> arRenderUtils.getRainbowOpaque(cRainbowSecValue.get(), saturationValue.get(), brightnessValue.get(), i * distanceValue.get())
                        "Sky" -> arRenderUtils.SkyRainbow(i * distanceValue.get(), saturationValue.get(), brightnessValue.get())
                        "LiquidSlowly" -> ColorsAR.LiquidSlowly(System.nanoTime(), i * distanceValue.get(), saturationValue.get(), brightnessValue.get())!!.rgb
                        "Fade" -> ColorUtils.fade(Color(redValue.get(), greenValue.get(), blueValue.get()), i * distanceValue.get(), 100).rgb
                        else -> cColor
                    },
                    when (rainbowType) {
                        "CRainbow" -> arRenderUtils.getRainbowOpaque(cRainbowSecValue.get(), saturationValue.get(), brightnessValue.get(), (i + 1) * distanceValue.get())
                        "Sky" -> arRenderUtils.SkyRainbow((i + 1) * distanceValue.get(), saturationValue.get(), brightnessValue.get())
                        "LiquidSlowly" -> ColorsAR.LiquidSlowly(System.nanoTime(), (i + 1) * distanceValue.get(), saturationValue.get(), brightnessValue.get())!!.rgb
        ...
        if (borderValue.get()) {
            ...
            // vertical
            arRenderUtils.drawRoundedRect(halfSize - strength, 0F, halfSize + strength, size,bordRad.get(), borderColor)

            // horizontal
            arRenderUtils.drawRoundedRect(0F, halfSize - strength, halfSize - strength, halfSize + strength,bordRad.get(), borderColor)
            arRenderUtils.drawRoundedRect(halfSize + strength, halfSize - strength, size, halfSize + strength,bordRad.get(), borderColor)

            glColor4f(1.0f, 1.0f, 1.0f, 1.0f)
        }
        // end

        arRenderUtils.makeScissorBox(x.toFloat(), y.toFloat(), x.toFloat() + ceil(size), y.toFloat() + ceil(size))
