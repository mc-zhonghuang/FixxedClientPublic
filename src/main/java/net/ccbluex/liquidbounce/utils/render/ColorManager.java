...
public class ColorManager {
    ...
    public  static Color red(int delay,long timeOffset,float sa){
        double red = Math.ceil((double) (System.currentTimeMillis() + (long) delay) / 20.0D);
        return Color.RED;
    }
    ...
    public static int RED(int delay, long timeOffset,float sa) {
        double rainbowState = Math.ceil((System.currentTimeMillis() + timeOffset) / 8 + delay / 20.0D);
        rainbowState %= 360.0;
        return Color.getHSBColor((float) (rainbowState / 360f), sa, 1).getRed();
    }
