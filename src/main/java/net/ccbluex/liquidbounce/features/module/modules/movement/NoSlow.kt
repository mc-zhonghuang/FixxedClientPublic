...
class NoSlow : Module() {
    private val modeValue = ListValue("PacketMode", arrayOf("Vanilla", "LiquidBounce", "Custom", "WatchDog","OldWatchDog", "OldWatchDog2", "NCP", "AAC", "AAC5", "Matrix", "Vulcan"), "Vanilla")
    ...
    private val testValue = BoolValue("SendPacket", false).displayable {  modeValue.get().equals("WatchDog") }
    /*new*/ private val debugValue = BoolValue("Debug", false).displayable{ modeValue.get().equals("WatchDog", true) }
    ...
    fun onMotion(event: MotionEvent) {
        ...
        val killAura = LiquidBounce.moduleManager[Aura::class.java]!!
    ...
    private val isBlocking: Boolean
        get() = (mc.thePlayer.isUsingItem || LiquidBounce.moduleManager[Aura::class.java]!!.blockingStatus) && mc.thePlayer.heldItem != null && mc.thePlayer.heldItem.item is ItemSword
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (modeValue.get().equals("WatchDog", true) && event.packet is S30PacketWindowItems && (mc.thePlayer.isUsingItem() || mc.thePlayer.isBlocking())) {
            ...
            if (debugValue.get())
                ClientUtils.displayChatMessage("检测到重置项目数据包")
        }
