...
import net.ccbluex.liquidbounce.utils.misc.HttpUtils2;
import net.ccbluex.liquidbounce.utils.timer.MSTimer;
import net.ccbluex.liquidbounce.value.BoolValue;
import net.ccbluex.liquidbounce.value.IntegerValue;
import net.ccbluex.liquidbounce.value.IntegerValue3;
...
public class BanChecker extends Module {
    ...
    public final IntegerValue3 alertTimeValue = new IntegerValue3("Alert-Time", 10, 1, 50, " seconds");
            ...
            public void run() {
                ...
                while (true) {
                    if (checkTimer.hasTimePassed(60000L)) {
                        try {
                            String apiContent = HttpUtils2.get(API_PUNISHMENT);
                            ...
                            if (jsonObject.get("success").getAsBoolean() && jsonObject.has("record")) {
                                ...
                                if (LiquidBounce.moduleManager.getModule(BanChecker.class).getState() && alertValue.get() && mc.thePlayer != null && (!serverCheckValue.get() || isOnHypixel()))
                                    if (STAFF_BAN_LAST_MIN > 0) {
                                        LiquidBounce.hud.addNotification(new Notification("BanChecker","Staffs banned " + STAFF_BAN_LAST_MIN+" players in the last minute!",STAFF_BAN_LAST_MIN > 3 ? NotifyType.ERROR : NotifyType.WARNING, 3,(int) (alertTimeValue.get() * 1000L)));
                                    }
                                    else{
                                        LiquidBounce.hud.addNotification(new Notification("BanChecker","Staffs didn't ban any player in the last minute.",NotifyType.SUCCESS, 3,(int) (alertTimeValue.get() * 1000L)));
                                    }

                                // 监管机构的禁令无关紧要，如果你想添加它，请打开一个问题。
                            }
                        } catch (Exception e) {
                            e.printStackTrace();

                            if (LiquidBounce.moduleManager.getModule(BanChecker.class).getState() && alertValue.get() && mc.thePlayer != null && (!serverCheckValue.get() || isOnHypixel()))
                                LiquidBounce.hud.addNotification(new Notification("An error has occurred.","",NotifyType.ERROR, 1000,(int) (alertTimeValue.get() * 1000L)));
                        }
                        checkTimer.reset();
                    }
                }
