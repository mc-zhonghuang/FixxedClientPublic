...
import net.ccbluex.liquidbounce.utils.misc.FP
...
class NoFall : Module() {
    val modeValue = ListValue("Mode", arrayOf("SpoofGround", "Hypixel", "AlwaysSpoofGround", "NoGround", "Packet", "Packet1", "Packet2", "MLG", "OldAAC", "LAAC", "AAC", "AAC3.3.11", "AAC3.3.15", "AACv4", /*"AAC5.0.4", */"AAC5.0.14", "Spartan", "CubeCraft", "BlocksMC", "HypSpoof", "Verus", "Damage", "MotionFlag",/*"OldMatrix", */ "Matrix", "MatrixPacket"), "SpoofGround")
    private val phaseOffsetValue = IntegerValue("PhaseOffset", 1, 0, 5)
    private val minFallDistance = FloatValue("MinMLGHeight", 5f, 2f, 50f)
    private val flySpeed = FloatValue("MotionSpeed", -0.01f, -5f, 5f)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (BlockUtils2.collidelock(mc.thePlayer.entityBoundingBox) { it is BlockLiquid } || BlockUtils2.collidelock(AxisAlignedBB(mc.thePlayer.entityBoundingBox.maxX, mc.thePlayer.entityBoundingBox.maxY, mc.thePlayer.entityBoundingBox.maxZ, mc.thePlayer.entityBoundingBox.minX, mc.thePlayer.entityBoundingBox.minY - 0.01, mc.thePlayer.entityBoundingBox.minZ)) { it is BlockLiquid }) {
            return
        }
...
//                } else { 1f }
            ...
            "aac" -> {
                if (mc.thePlayer.fallDistance > 2f) {
                    mc.netHandler.addToSendQueue(C03PacketPlayer(true))
                    aacstate = 2
                } else if (aacstate == 2 && mc.thePlayer.fallDistance < 2) {
        ...
        if (modeValue.equals("AACv4") && event.eventState === EventState.PRE) {
            ...
            if (event.eventState == EventState.PRE) {
                ...
                if (mc.thePlayer.fallDistance > minFallDistance.get()) {
                    val fallingPlayer = FP(mc.thePlayer)
