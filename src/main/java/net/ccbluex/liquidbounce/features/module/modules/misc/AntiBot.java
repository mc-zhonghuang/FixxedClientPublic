...
import net.ccbluex.liquidbounce.utils.render.ColorUtils2;
...
public final class AntiBot extends Module {
    public static final ListValue modeValue = new ListValue("Mode", new String[] {"Normal", "AntiCheat", "Hypixel", "Mineplex", "Syuu", "TestMatrix", "Matrix", "Huayuting"}, "Normal");
    ...
    public void onUpdate(UpdateEvent updateEvent) {
        if(sendTimer.hasReached(3000)) {
            if(removedBots.size() != 0)
                ClientUtils2.displayChatMessage(("\u00a77[\u00a79BotKiller\u00a77] \u00a79Removed " + removedBots.size() + " Bots"));
            removedBots.clear();
            sendTimer.reset();
        }
            ...
            case "Syuu": {
                for (EntityPlayer i : mc.theWorld.playerEntities) {
                    ...
                    if(i.getHealth() == 3838 && stacks[1].getItem() instanceof ItemFood) {
                        ClientUtils2.displayChatMessage(("\u00a77[\u00a79BotKiller\u00a77] \u00a79Removed a bot: \u00a74" + i.getName() + " \u00a79Coords \u00a7bX:\u00a7f" + Math.round(i.posX) + "\u00a7bY:\u00a7f" + Math.round(i.posY) + "\u00a7bZ:\u00a7f" + Math.round(i.posZ)));
                        mc.theWorld.removeEntity(i);
                    }
                }
                break;
                    ...
                    for(Entity i : mc.theWorld.loadedEntityList) {
                        ...
                        for(S38PacketPlayerListItem.AddPlayerData j : ((S38PacketPlayerListItem) packet).getEntries()) {
                            if(!(entityLivingBase instanceof EntityPlayerSP) && j.getProfile().getName().equalsIgnoreCase(entityLivingBase.getName()) && (j.getProfile().getId() != entityLivingBase.getUniqueID() || j.getProfile().getId() != entityLivingBase.getPersistentID())) {
                                ClientUtils2.displayChatMessage(("\u00a77[\u00a79BotKiller\u00a77] \u00a79Removed a bot: \u00a74" + j.getProfile().getName()));
                                event.cancelEvent();
                            }
                        }
                    }
    ...
    public static boolean isBot(final EntityLivingBase entity) {
        if(modeValue.get().equals("Normal")) {
            ...
            if (antiBot.tabValue.get()) {
                ...
                final String targetName = ColorUtils2.stripColor(entity.getDisplayName().getFormattedText());

                if (targetName != null) {
                    for (final NetworkPlayerInfo networkPlayerInfo : mc.getNetHandler().getPlayerInfoMap()) {
                        final String networkName = ColorUtils2.stripColor(EntityUtils2.getName(networkPlayerInfo));
            ...
            if (antiBot.duplicateInTabValue.get()) {
                if (mc.getNetHandler().getPlayerInfoMap().stream()
                        .filter(networkPlayer -> entity.getName().equals(ColorUtils2.stripColor(EntityUtils2.getName(networkPlayer))))
                        .count() > 1)
                    return true;
            }
