...
import net.ccbluex.liquidbounce.utils.block.PlaceInfo2;
import net.ccbluex.liquidbounce.utils.render.*;
import net.ccbluex.liquidbounce.utils.timer.MSTimer;
import net.ccbluex.liquidbounce.utils.timer.TimeUtils2;
...
public class Blockfly extends Module {
    ...
    private final BoolValue safeWalkValue = new BoolValue("SafeGround", true);
    ...
    // Visuals
    public final ListValue counterDisplayValue = new ListValue("Counter", new String[]{"Off", "Default","Simple", "Advanced", "Sigma", "Novoline"}, "Default");
    ...
    private final BoolValue onlynotauraValue = new BoolValue("Off-Aura",false);
    private final BoolValue onlynotvelocityValue = new BoolValue("Off-Velocity",false);
    private final BoolValue onlynotSpeedValue = new BoolValue("Off-Speed",true);
    private final BoolValue autoopenAuraValue = new BoolValue("AutoOpen-Aura",false);
    private final BoolValue autoopenVelocityValue = new BoolValue("AutoOpen-Velocity",false);
    private final BoolValue autoopenSpeedValue = new BoolValue("AutoOpen-Speed",false);
    private boolean autoopenAura = false;
    private boolean autoopenVelocity = false;
    private boolean autoopenSpeed = false;
    /**
     * MODULE
     */
    //counter
    ...
    public void onEnable() {
        if(onlynotauraValue.get()){
            if(LiquidBounce.moduleManager.getModule(Aura.class).getState()){
                LiquidBounce.moduleManager.getModule(Aura.class).setState(false);
                if(autoopenAuraValue.get()){
                    autoopenAura = true;
                }
            }
            if(LiquidBounce.moduleManager.getModule(Velocity.class).getState()){
                LiquidBounce.moduleManager.getModule(Velocity.class).setState(false);
                if(autoopenVelocityValue.get()){
                    autoopenVelocity = true;
                }
            }
            if(LiquidBounce.moduleManager.getModule(Speed.class).getState()){
                LiquidBounce.moduleManager.getModule(Speed.class).setState(false);
                if(autoopenSpeedValue.get()){
                    autoopenSpeed = true;
                }
            }
        }
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if (mc.thePlayer.onGround) {
            ...
            if (zitterValue.get() && zitterModeValue.get().equalsIgnoreCase("teleport")) {
                MovementUtils3.strafe(zitterSpeed.get());
        ...
        if (shouldGoDown) launchY = (int) mc.thePlayer.posY - 1;
            ...
            if ((!autoJumpValue.get() && !(smartSpeedValue.get() && LiquidBounce.moduleManager.getModule(Speed.class).getState())) || GameSettings.isKeyDown(mc.gameSettings.keyBindJump) || mc.thePlayer.posY < launchY)
                launchY = (int) mc.thePlayer.posY;
            if (autoJumpValue.get() && !LiquidBounce.moduleManager.getModule(Speed.class).getState() && MovementUtils3.isMoving() && mc.thePlayer.onGround && mc.thePlayer.jumpTicks == 0) {
                mc.thePlayer.jump();
    ...
    public void onMotion(final MotionEvent event) {
        ...
        if (rotationTimer.hasTimePassed(3000L) && debugValue.get()) {
            ClientUtils2.displayChatMessage("§8[§9§lDebug§8] RotationYaw:" + mc.thePlayer.rotationYaw + " | RotationPitch:" + mc.thePlayer.rotationPitch);
            rotationTimer.reset();
        }
        // Lock Rotation
    ...
    private void update() {
        ...
        if (!autoBlockValue.get().equalsIgnoreCase("Off") ? InventoryUtils2.findAutoBlockBlock() == -1 && !isHeldItemBlock : !isHeldItemBlock)
            return;
    ...
    private void place() {
        ...
        if (mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, itemStack, targetPlace.getBlockPos(), targetPlace.getEnumFacing(), targetPlace.getVec3())) {
            delayTimer.reset();
            delay = TimeUtils2.randomDelay(minDelayValue.get(), maxDelayValue.get());
            if (mc.thePlayer.onGround) {
    ...
    public void onDisable() {
        if(autoopenAura){
            LiquidBounce.moduleManager.getModule(Aura.class).setState(true);
            autoopenAura = false;
        }
        if(autoopenVelocity){
            LiquidBounce.moduleManager.getModule(Velocity.class).setState(true);
            autoopenVelocity = false;
        }
        if(autoopenSpeed){
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        final String info2 = getBlocksAmount() + "";
        ...
        if (counterMode.equalsIgnoreCase("advanced")) {
            ...
            if (blurValue.get())
                BlurUtils2.blurArea(scaledResolution.getScaledWidth() / 2 - (infoWidth / 2) - 4, scaledResolution.getScaledHeight() / 2 - 39, scaledResolution.getScaledWidth() / 2 + (infoWidth / 2) + 4, scaledResolution.getScaledHeight() / 2 - (canRenderStack ? 5 : 26), blurStrength.get());
        ...
        if (counterMode.equalsIgnoreCase("default")){
            ...
            GL11.glVertex2d(
                    (scaledResolution.getScaledWidth() / 2 - 3),
                    (scaledResolution.getScaledHeight() - 60)
            );
            GL11.glVertex2d(
                    (scaledResolution.getScaledWidth() / 2),
                    (scaledResolution.getScaledHeight() - 57)
            );
            GL11.glVertex2d(
                    (scaledResolution.getScaledWidth() / 2 + 3),
                    (scaledResolution.getScaledHeight() - 60)
            ...
            RenderUtils.drawRoundedCornerRect(
                    (scaledResolution.getScaledWidth() / 2 - infoWidth / 2 - 4),
                    (scaledResolution.getScaledHeight() - 60),
                    (scaledResolution.getScaledWidth() / 2 + infoWidth / 2 + 4),
                    (scaledResolution.getScaledHeight() - 74),
                    2.5f,
                    new Color(0.25f, 0.25f, 0.25f, progress).getRGB()
            );
            GlStateManager.resetColor();
            Fonts.font35.drawCenteredString(
                    info, scaledResolution.getScaledWidth() / 2 + 0.1f,
                    (scaledResolution.getScaledHeight() - 70), new Color(1f, 1f, 1f, 0.8f * progress).getRGB(), false
            );
            GlStateManager.translate(0f, 14f + progress * 4f, 0f);
        }
        if (counterMode.equalsIgnoreCase("sigma")) {
            ...
            GlStateManager.resetColor();
            Fonts.font40.drawCenteredString(info, scaledResolution.getScaledWidth() / 2 + 0.1F, scaledResolution.getScaledHeight() - 70, new Color(1F, 1F, 1F, 0.8F * progress).getRGB(), false);
            GlStateManager.translate(0, 14F + (progress * 4F), 0);
        }
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        for (int i = 0; i < (modeValue.get().equalsIgnoreCase("Expand") ? expandLengthValue.get() + 1 : 2); i++) {
            ...
            final PlaceInfo placeInfo = PlaceInfo2.get(blockPos);
    ...
    private int getBlocksAmount() {
        ...
        for (int i = 36; i < 45; i++) {
            ...
            if (itemStack != null && itemStack.getItem() instanceof ItemBlock) {
                ...
                if (mc.thePlayer.getHeldItem() == itemStack || !InventoryUtils2.BLOCK_BLACKLIST.contains(block))
